...
class GoogleMapsLoader extends React.Component<GoogleMapsLoaderProps, GoogleMapsLoaderState> {
  ...
  constructor(props: GoogleMapsLoaderProps) {
    ...
    console.log('Initializing Google Maps Loader with API key:', apiKey ? `${apiKey.substring(0, 10)}...` : 'Not set');
  ...
  async componentDidMount() {
    ...
    if (!process.env.REACT_APP_GOOGLE_MAPS_API_KEY) {
      console.warn('Google Maps API key not found. Running in development mode without maps.');
      this.setState({ 
        error: 'Google Maps API キーが設定されていません。開発モードで実行中です。' 
      });
      return;
    }
