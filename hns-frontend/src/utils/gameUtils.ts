...
export const generateRandomCharacterPositions = (characters: Character[], gameArea?: google.maps.LatLngBounds): Character[] => {
  ...
  const defaultLocations = [
    ...
    { lat: 34.6876, lng: 135.5260, name: '大阪城' },
...
// 指定位置の周辺にランダムな位置を生成
export const generateRandomPositionNearby = (center: { lat: number; lng: number }, radiusInMeters: number) => {
  ...
  const deltaLng = (distance * Math.sin(angle)) / (earthRadius * Math.cos(center.lat * Math.PI / 180)) * (180 / Math.PI);
