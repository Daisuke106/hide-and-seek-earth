...
export const generateRandomStreetViewPosition = (basePosition: google.maps.LatLngLiteral, radius: number = 50): google.maps.LatLngLiteral => {
  ...
  const deltaLng = (distance * Math.sin(angle)) / (earthRadius * Math.cos(basePosition.lat * Math.PI / 180)) * (180 / Math.PI);
...
export const generateCharacterStreetViewPositions = (characters: Character[], selectedPosition: google.maps.LatLngLiteral): Character[] => {
