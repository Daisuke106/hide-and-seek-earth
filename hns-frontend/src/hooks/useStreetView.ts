...
interface UseStreetViewReturn {
  ...
  isLoading: boolean;
  error: string | null;
  initializeStreetView: (element: HTMLElement, position: google.maps.LatLngLiteral) => Promise<void>;
...
export const useStreetView = (): UseStreetViewReturn => {
  ...
  const initializeStreetView = useCallback(async (
    ...
    try {
      ...
      const streetViewInstance = await googleMapsService.createStreetViewPanorama(
        element,
        streetViewOptions
      );
